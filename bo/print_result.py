...
for i in range(1, 11):
    for j in range(5):
        fn = 'guided_results%d/scores%d.dat' % (i,j)
        scores = load_object(fn)
        fn = 'guided_results%d/valid_smiles%d.dat' % (i,j)
        smiles = load_object(fn)
        all_smiles.extend(zip(smiles, scores))

all_smiles = [(x, -y) for x, y in all_smiles]
all_smiles = sorted(all_smiles, key=lambda x:x[1], reverse=False)
for s, v in all_smiles[-20:]:
    print(s, v)
